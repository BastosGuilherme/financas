...
export default function Home() {
  ...
  function renderOverview() {
    ...
    return <>
      ...
      <section className="hero-card"><div className="hero-copy"><div className="hero-label"><CircleDollarSign size={17} /> Saldo disponível total</div><strong>{formatMoney(available)}</strong><p>Banco + Flash, descontando as faturas abertas</p><div className="hero-breakdown"><div><small>Banco</small><b>{formatMoney(bankBalance)}</b></div><div><small>Flash</small><b>{formatMoney(flashBalance)}</b></div></div></div><div className="hero-aside"><div className="hero-avatars"><span>G</span><span>F</span></div><div><small>Visão do casal</small><b>Atualizado agora</b></div></div><div className="hero-sparkline" aria-hidden="true"><span /><span /><span /><span /><span /><span /><span /></div></section>
